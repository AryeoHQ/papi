...
class PrivateController extends PapiController
{
    ...
    public function preparePublicSpec($spec_path, $out_path, $overrides_path, $public_customizations)
    {
        ...
        $array = $this->keepExternalPaths($array);
    ...
    public function keepExternalPaths($array)
    {
            ...
            foreach ($array['paths'] as $path_key => $path) {
                ...
                foreach ($path as $method_key => $method) {
                    if (in_array($method_key, $valid_methods, true)) {
                        if (isset($method['x-external']) && $method['x-external'] === true) {
                            // keep external paths...
                        } else {
                            // unset internal paths...
